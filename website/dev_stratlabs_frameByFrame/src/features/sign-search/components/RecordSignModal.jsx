...
*/
  ...
  const endRecordingAsync = async () => {
    const data = await stopRecordingHolistic();
  ...
  useEffect(() => {
    if (processingFrames) {
      //timeout para que la interfaz pueda actualizarse y mostrar la pantalla de procesando, antes que inicie el procesamiento intenso del holitic.
      setTimeout(endRecordingAsync, 10);
    }
  }, [processingFrames]);
  ...
  useEffect(() => {
    console.log(
      "processingHolisticFrame : " + processingHolisticFrame.current + " | " + progressHolisticFrame
    );
  }, [processingHolisticFrame.current, progressHolisticFrame]);
                ...
                {processingFrames && (
                  ...
                  >
                    ...
                    <div style={{ zIndex: 100, textAlign: "center" }}>
                      <div style={{ color: "#ffffffaa" }}>Procesando...</div>
