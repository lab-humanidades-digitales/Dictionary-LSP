...
export const useHolistic = () => {
    ...
    const processingHolisticFrame = useRef(false);
    //const progressHolisticFrame = useRef(false);
    const [progressHolisticFrame, setProgressHolisticFrame] = useState(0);
    ...
    const onHolisticResults = (results) => {
        ...
        if (processingHolisticFrame.current)
    ...
    const initCamera = async (deviceId) => {
        ...
        cameraRef.current = new Camera(webcamRef.current.video, {
            onFrame: async () => {
                ...
                if (!processFrame.current) return;
                if (processingHolisticFrame.current) return;
    ...
    const startRecording = () => {
        ...
        recordedMedia.current = [];
        recordedLandmarks.current = [];
        processingHolisticFrame.current = false;
    ...
    const stopRecording = async () => {
        ...
        var offlineRecording = [];

        if (cameraRef.current && cameraRef.current.startRecording) {
            offlineRecording = cameraRef.current.stopRecording();
        }
        ...
        processingHolisticFrame.current = true;
        for (var i = 0; i < offlineRecording.length; i++) {
            await holisticRef.current.send({ image: offlineRecording[i] });
        }
        processingHolisticFrame.current = false;
    ...
    return {
        ...
        progressHolisticFrame,
        processingHolisticFrame,
    };
};
